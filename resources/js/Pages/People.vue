      ...
      >
        <template v-slot:top>
          <v-toolbar flat>
            ...
            <v-dialog v-model="assignTeamDialog" max-width="500px" persistent>
              <v-card>
                <form
                  @submit.prevent="
                    isSelectedPeople ? assignTeams() : assignTeam()
                  "
                >
                  <v-card-title class="text-h5">Assign Team</v-card-title>
                  <v-card-text>
                    <v-autocomplete
                      ...
                      :error-messages="selectedTeamsError"
                    >
                    </v-autocomplete>
                  </v-card-text>
                  <v-card-actions>
        ...
        <template v-slot:item.avatar="{ item }">
          <v-avatar size="30">
            <img
              :src="`/storage/avatar/${
                item.avatar ? `user/${item.avatar}` : 'default.jpg'
              }`"
            />
          </v-avatar>
        </template>
        ...
        <template v-slot:item.status="{ item }">
          ...
          <div class="status off" v-else></div>
        </template>
...
export default {
  ...
  created() {
    this.dispatchUser({ type: "ADD_PEOPLE", payload: this.people });
  ...
  methods: {
    ...mapActions(["dispatchUser", "dispatchTeam", "dispatchNotifications"]),
    sendInvite() {
